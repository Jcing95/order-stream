...
use crate::backend::event::CreateEvent;

#[component]
pub fn CreateEvent() -> impl IntoView {
    let create_action = ServerAction::<CreateEvent>::new();

    // Handle successful event creation
    Effect::new(move |_| {
        if let Some(Ok(_event)) = create_action.value().get() {
            // Event created successfully - websocket broadcast will update the list
        }
    });
    ...
    view! {
        <div class="max-w-md w-full space-y-8">
            ...
            >
                <div class="space-y-4">
                    <div>
                        <label for="name" class="block text-sm font-medium text-text mb-2">"Event Name"</label>
                        <input
                            id="name"
                            name="req[name]"
                            type="text"
